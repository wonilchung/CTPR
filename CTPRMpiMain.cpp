...
void CTPRMpiProc(arma::field<arma::fvec> & Ys, arma::field<arma::fmat> & Xs, const arma::fcube & A, arma::field<arma::fvec> & lambda1, arma::fvec lambda2,
...
void CTPRMpiMSE(arma::fmat & mse, arma::field<arma::fmat> & b_est, arma::field<arma::fvec> & lambda1, 
...
// Main procedure
void CTPRMpiTune(arma::field<arma::fvec> & Ys, arma::field<arma::fmat> & Xs, const arma::fcube & A,
...
void CTPRMpiPredProc(arma::fvec & Ys_test, arma::fmat & Xs_test, arma::fvec & b_min, arma::fvec & b_min0, 
...
void CTPRMpiPred(arma::fvec & Ys_test, arma::fmat & Xs_test, arma::fvec & b_min, arma::fvec & b_min0, arma::field<arma::fmat> & b_est, 
	arma::fmat & cvm, arma::fvec & ctprRes, PARAM & cPar){
	...
	if (rank == 0) ctprRes.save(cPar.output+".res", arma::csv_ascii);
	//if (rank == 0) ctprRes.save("./res/mpires_" + fnameYs + "_" + penalize + "_" + patch::to_string(cPar.nGroup) + "_" + patch::to_string(size) + ".txt", arma::csv_ascii);
	...
	if (rank == 0) {
	  ...
	  std::cout << "**************************************************************" << std::endl;
		...
		std::cout << ",Slope=" << slope0 << ",Nzbeta=" << nzbe0 << ",cvMSE=" << ctprRes(5) << std::endl;
		std::cout << ctprRes(3) << " " << R20 << " " << R20_1 << " " << mse0 << " " << slope0 << " " << nzbe0 << " " << ctprRes(5) << std::endl;
		std::cout << std::endl;
		...
		std::cout << ",Slope=" << slope << ",Nzbeta=" << nzbe << ",cvMSE=" << ctprRes(12) << std::endl;
		std::cout << ctprRes(9) << " " << R2 << " " << R2_1 << " " << mse << " " << slope << " " << nzbe << " " << ctprRes(12) << std::endl;
		std::cout << std::endl;
		std::cout << "(3) All Prediction Results (lambda2=" << ctprRes(10) << ")" << std::endl;
